...
/* A handful of Commons portraits are PNGs. Lossless encoding of a photograph
   ...
   gets re-requested at smaller widths until it fits.

   Known limit: the ladder helps but cannot fix PNG. Measured on one file,
   420px/1536KB -> 320px/676KB -> 240px/392KB -> 200px/392KB; it stops improving
   while still over budget, because re-encoding a photograph as PNG at any size
   is simply expensive. Shipping a slightly heavy lazy-loaded portrait beats
   dropping the driver's face or taking on an image-processing dependency, so
   over-budget files are kept and reported rather than rejected. */
...
/* A cached file counts as good if it fits the budget, or if the shrink ladder
   already ran and could not get it under — re-fetching that one every build
   would cost a request an hour forever and end up at the same bytes. Without
   any size check at all, a file downloaded before the budget existed would be
   cached as fine indefinitely, which is the other half of the same bug. */
async function cachedFileIsUsable(p, credit) {
  try {
    const info = await stat(p);
    return info.size <= MAX_BYTES || credit.minimised === true;
  } catch {
    return false;
  }
}
...
export async function ensurePortraits(drivers, { assetsDir, dataDir }) {
  ...
  for (const driver of drivers) {
    ...
    if (
      cached &&
      (cached.unavailable ||
        (cached.file && (await cachedFileIsUsable(path.join(assetsDir, cached.file), cached))))
    ) {
      continue;
    }
    ...
    try {
      ...
      const bytes = await downloadWithinBudget(found, path.join(assetsDir, rel));

      credits[driver.id] = {
        file: rel,
        bytes,
        minimised: bytes > MAX_BYTES,
        author: found.author,
  ...
  for (const [id, credit] of Object.entries(credits)) {
    if (credit.unavailable || !credit.file) continue;
    try {
      const info = await stat(path.join(assetsDir, credit.file));
      if (info.size > MAX_BYTES) heavy.push(`${id} ${Math.round(info.size / 1024)}KB`);
    } catch {
      /* reported elsewhere */
    }
  }
  if (heavy.length) console.log(`  portraits over budget: ${heavy.join(', ')}`);
